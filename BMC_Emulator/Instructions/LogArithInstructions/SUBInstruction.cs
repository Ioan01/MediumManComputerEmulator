...
namespace BMC_Emulator
{
    ...
    {
        public SUBInstruction(GroupCollection matchGroups) : base(matchGroups)
        {
        }

        public SUBInstruction(short word) : base(word)
        {
        }
        ...
        {
            // Get the index of the register to which we will subtract the value
            var regToBeStored = emulator.Registers[GetBits(10, 10)];
            // Get the value that will be subtracted from the register
            var valueToSubtract = 0;

            // If the 9th bit is set to 1, the value to subtract is the value in the register whose index is specified in the last two bits of the instruction
            if (GetBits(9, 9) == 1)
            {
                valueToSubtract = GetBits(0, 8);
            }
            else
            {
                // Otherwise, the value to substract is the value specified in the last 8 bits of the instruction
                valueToSubtract = emulator.Registers[GetBits(0, 1)];
            }
            
            var result = (ushort)(regToBeStored - valueToSubtract);
