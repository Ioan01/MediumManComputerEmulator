...
namespace BMC_Emulator
{
    ...
    {
        public ADDInstruction(GroupCollection matchGroups) : base(matchGroups)
        {
        }

        public ADDInstruction(short word) : base(word)
        {
        }
        ...
        public override void Execute(Emulator emulator)
        {
            ...
            // Get the value that will be added to the register
            var valueToAdd = 0;
            ...
            if (GetBits(9, 9) == 1)
            {
                valueToAdd = GetBits(0, 8);
            }
            else
            {
                // Otherwise, the value to add is the value specified in the last 8 bits of the instruction
                valueToAdd = emulator.Registers[GetBits(0, 1)];
            }

            var result = (ushort)(regToBeStored + valueToAdd);
